...
define ['../base-directive','text!./style.css', 'text!./view.html', 'underscore', "moment"], (base, css, view, _, moment) ->
  class HeaderPredictDirective extends base.BaseDirective
    ...
    link: (scope, element, attrs) =>
      ...
      scope.$watch '$root.path',(path)=>  # 设置 站点页header和侧菜单颜色
        return if not path
        if scope.$root.path.indexOf('station-info')>-1
          scope.stationPageFlag = true
        else
          scope.stationPageFlag = false
