        ...
        notification.query {user: scope.project.model.user, project: scope.project.model.project}, null, (err, rules)->
          _.map rules, (rule)->
            if rule.notification.substr(0,4) is "link"
              ...
              if not triggerEquip
                console.log("error:triggerEquip is null",triggerEquip)
                return false
...
#        console.log(scope.re.cmdModelParameters)
...
#          console.log("setParameterContent:",result)
          return result
