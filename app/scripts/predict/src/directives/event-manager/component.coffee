...
#      "stationId":"="
    ...
    show: (scope, element, attrs) =>
      ...
      # reset all count
        ...
        # count predicts
        for predict in scope.predicts
          predictStatistic = predict.statistic
          predictCounts = predictStatistic.counts
          predictSeverities = predictStatistic.severities

          # count by event type
          predictStatistic.types ?= {}

          for station in predict.stations
            stationStatistic = station.statistic
            stationCounts = stationStatistic.counts

            predictCounts.startEvents += stationCounts.startEvents
            predictCounts.endEvents += stationCounts.endEvents
            predictCounts.confirmedEvents += stationCounts.confirmedEvents
            predictCounts.allEvents += stationCounts.allEvents

            stationSeverities = stationStatistic.severities
            for k, v of stationSeverities
              predictSeverities[k] = (predictSeverities[k] ? 0) + v

            if predictStatistic.severity < stationStatistic.severity
              predictStatistic.severity = stationStatistic.severity
            #            predictStatistic.color = stationStatistic.color

            for key, type of stationStatistic.types
              if predictStatistic.types.hasOwnProperty type.type
                statisticType = predictStatistic.types[type.type]
                statisticType.count += type.count
                statisticType.severity = type.severity if type.severity > statisticType.severity
              else
                predictStatistic.types[type.type] =
                  type: type.type
                  count: type.count
...
#            console.log(msg)
      ...
      scope.selectPredict=(predict)=>
        return if not predict
        scope.predict = predict
#        stationId = predict.stations[0]?.model?.station
        stationId = predict.model.station
        changeStation(stationId)
        scope.$applyAsync()
      ...
      scope.selectParent(scope.predicts[0])
      scope.selectChild(scope.predict?.stations[0] || scope.project?.stations?.items[0])
      scope.selectPredict(scope.predicts[0])
