...
define ['../base-directive','text!./style.css', 'text!./view.html', 'underscore', "moment"], (base, css, view, _, moment) ->
  class InventoryListDirective extends base.BaseDirective
    ...
    show: ($scope, element, attrs) =>
      ...
      $scope.jumpImg = @getComponentPath('image/next.png')
...
# add a random number to enforce to refresh
      ...
      processTypes = (data, refresh) =>
        ...
        if !$scope.currentType
          # 默认让选择电机类型
          $scope.currentType = _.find typesArr,(item)->item.type is 'motor'
...
#station: $scope.equipment.model.station
      ...
      #跳转到设备详情页面
      $scope.lookData2 = (equipment) =>
        window.location.hash = "#/device-details/#{$scope.project.model.user}/#{$scope.project.model.project}/#{equipment.model.station}/#{equipment.model.equipment}"
      #跳转到设备监控页面
      $scope.lookData3 = (equipment) =>
        window.location.hash = "#/monitoring/#{$scope.project.model.user}/#{$scope.project.model.project}?&station=#{equipment.model.station}&equipment=#{equipment.model.equipment}&type=#{equipment.model.type}"
