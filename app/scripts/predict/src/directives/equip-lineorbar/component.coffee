...
define ['../base-directive','text!./style.css', 'text!./view.html', 'underscore', "moment"], (base, css, view, _, moment) ->
  class EquipLineorbarDirective extends base.BaseDirective
      ...
      scope.equipment.loadSignals null, (err, signallists) =>
        ...
        if signals and signals.value
          scope.signalList = _.map(JSON.parse(signals.value), (d, i) =>
            _signal = _.find(newSignalLists, (m) => m.model.signal == d.signal)
            return { signal: d.signal, name: _signal?.model.name, check: if i == 0 then true else false }
          )
        else
          scope.signalList = _.map(newSignalLists, (d, i) -> { signal: d.model.signal, name: d.model.name, check: if i == 0 then true else false })
